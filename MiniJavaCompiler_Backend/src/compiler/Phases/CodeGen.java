...
public class CodeGen extends IRElementVisitorWithArgument<CODE> {
	...
	public CODE visitStatement(MJIfElse e, CODE code) throws VisitorException {
		...
		visitExpression(e.getCondition(), code);
		code.pop(CODE.TMP0);
		LC3label elseStart = new LC3label();
		code.add(new LC3BRZ(elseStart));
		visitStatement(e.getIfBlock(), code);
		code.comment(" End of if block");
		LC3label ifelseEnd = new LC3label();
		code.add(new LC3BR(ifelseEnd));
		code.comment("Start of elseblock");
		code.add(elseStart);
	...
	public CODE visitStatement(MJWhile e, CODE code) throws VisitorException {
		...
		LC3label whileStart = new LC3label();
		LC3label whileEnd = new LC3label();
		code.add(whileStart); //Start Label
		visitExpression(e.getCondition(), code); //Check condition
		code.pop(CODE.TMP0); //Get result
		code.add(new LC3BRZ(whileEnd)); //Goto to end label if false
